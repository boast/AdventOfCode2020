    ...
    internal class Day04 : Day
    {
        ...
        private static IEnumerable<Passport> ParsePassports(IEnumerable<string> input)
        {
            string flattenInput = new string(input.SelectMany(line => 
                line == string.Empty 
                    ? "},{" // combine objects
                    : "\"" + line.Replace(":", "\":\"").Replace(" ", "\",\"") + "\"," // convert to json properties (_ is space). foo:bar_ -> "foo":"bar",
                    ).ToArray());
            
            string wrappedJson = $"[{{{flattenInput}}}]"; // wrap as array and start/finish the first/last object. "foo":"bar",}{"foo":"bar", -> [{"foo":"bar",}{"foo":"bar",}]

            var options = new JsonSerializerOptions
            {
                AllowTrailingCommas = true, // important
                PropertyNameCaseInsensitive = true, // only needed because we want c# style naming conventions in the Passport class
            };
